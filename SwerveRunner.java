...
@TeleOp(name="Diff", group="DriveModes")
public class Diff extends OpMode {
    ...
    */
    ...
    private double x, y, rotation, lateral, angle;
    ...
    public void init() {    
		...
		TR = hardwareMap.get(DcMotorEx.class, "TL"); //1
    	BR = hardwareMap.get(DcMotorEx.class, "BL"); //1
		...
		Module left = new Module(145.1, 106.0/13 * 18.0/14, 1);
		Module right = new Module(145.1, 106.0/13 * 18.0/14, -1);
    ...
    public void loop() {
        ...
        left.setEncoders(TL.getCurrentPosition(), BL.getCurrentPosition());
		right.setEncoders(TR.getCurrentPosition(), BR.getCurrentPosition());
...
class Module {
    ...
    private final double MOTOR_TICKS;
    private final double RATIO;

	public Module(double MOTOR_TICKS, double RATIO, int side){
		this.MOTOR_TICKS = MOTOR_TICKS;
		this.RATIO = RATIO;
        this.side = side;
	}
	public void setEncoders(double topTick, double bottomTick){
